...
function startEngine({ mock, skill, map }) {
  ...
  var skipRows = 0;
  send(`s ${SNAP_COLS} ${SNAP_ROWS}`);   // tiny frame stream just to keep S/L/E flowing
  send("m mono");
  send("f 5");
  return useFake;
}
...
server.registerTool("doom_look", {
  ...
  inputSchema: {},
}, async () => ({ content: [{ type: "text", text: await look() }] }));
...
}, async ({ text }) => { send(`c ${text}`); await sleep(400); return { content: [{ type: "text", text: await look() }] }; });

server.registerTool("doom_stop", { title: "Stop Doom", description: "Quit the game.", inputSchema: {} },
  async () => { stopEngine(); return { content: [{ type: "text", text: "Doom stopped." }] }; });
